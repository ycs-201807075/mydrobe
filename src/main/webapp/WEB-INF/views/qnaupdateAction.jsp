...
<body>
	 <%
         ...
         if(!userID.equals(qnaboard.getUserID())){
	 	    ...
	 	    if (request.getParameter("qnaboardTitle") == null || request.getParameter("qnaboardContent") == null ||
	 	        request.getParameter("qnaboardTitle").equals("") || request.getParameter("qnaboardContent").equals("")){
	 		 PrintWriter script = response.getWriter();
	 		 script.println("<script>");
	 		 ...
	 		 script.println("</script>");
        	 	...
        	 	int result = qnaBoardDAO.update(request.getParameter("qnaboardTitle"),request.getParameter("qnaboardContent"));
	 		if (result == -1){
        	 		PrintWriter script = response.getWriter();
        	 		script.println("<script>");
        	 		...
        	 		PrintWriter script = response.getWriter();
        	 		script.println("<script>");
        	 		script.println("alert('글수정에 성공하셨습니다')");
